...
import psutil
import requests
from vlaa_gui.agent_core.utils.common_utils import box_iou
...
from vlaa_gui.agent_core.agents.grounding import ACI, agent_action
...
class WindowsACI(ACI):
    ...
    @agent_action
    def hover_at(self, element_id: int):
        """Move the cursor to the element without clicking.
        Args:
            element_id:int, ID of the element to hover over
        """
        node = self.find_element(element_id)
        coordinates: Tuple[int, int] = node["position"]
        sizes: Tuple[int, int] = node["size"]
    ...
    @agent_action
    def fail(self):
        """Report that the current task is infeasible and cannot be completed."""
        return """FAIL"""
